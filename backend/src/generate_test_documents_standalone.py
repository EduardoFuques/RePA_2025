...
def generate_documents():
    ...
    for user in TEST_USERS:
        ...
        if user['tipo'] == 'pf':
            ...
            filename = f"DNI_{user['apellido']}_{user['nombre']}_{uuid.uuid4().hex[:8]}.pdf"
            filepath = os.path.join(user_dir, filename)
            create_sample_file(filepath, f"DNI - {user['nombre']} {user['apellido']}", '.pdf')
            ...
            docs = [
                ...
                ("constancia_cuit", f"Constancia CUIT - {user['razon_social']}", '.pdf'),
                ("acta_autoridades", f"Acta de Autoridades - {user['razon_social']}", '.docx'),
                ("cv_institucional", f"CV Institucional - {user['razon_social']}", '.docx')
            ]
    ...
    print("   UPDATE personas_fisicas SET dni_adjunto_path = '<filename>' WHERE user_id = '<user_id>';")
    print("   UPDATE persona_juridica SET estatuto_path = '<filename>' WHERE user_id = '<user_id>';")
